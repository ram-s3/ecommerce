...
import { Form, json, redirect, useLoaderData, Link } from "react-router-dom";
import { gettoken } from "../../util/authtoken";
import { TextField, Stack, Card, FormLabel, Button } from "@mui/material";
...
export async function action({ request, params }) {
    ...
    if (request.method === "PATCH") {
        url = "http://localhost:8000/admin/productdetails/edit/" + id;
...
export async function loader({ params }) {
    ...
    const id = params.id;
    const productdetails = await fetch("http://localhost:8000/productdetails/" + id);
